...
|------------------|------------------|------------------|------------------|
| 1        | SiteStructureAddressPoint, query supplies a usable HNO     | The address point position, Z included where present             | ROOFTOP                   |
...
***✔ Settled (Session 16, decision 119) — rung 1 requires a house number in the QUERY, not merely in the record.** The rung column above now says so. Rungs 2 and 3 were always distinguished by whether there is a house number to interpolate; rung 1 was written as though it were distinguished only by layer, and it is not. An address point's identity is its house number (decision 69, which is why `Add_Number` is a gate rather than a scored term), so a query supplying none — or supplying one decision 63 dropped — has nothing for rung 1 to be an answer to, and the ladder begins at rung 2. See §6.1 and decision 119.*
...
***✔ Settled (Session 16, decision 119) — the sentence above is now true of the algorithm, not only of this section.** It has said "answered at rung 3" since Session 1 and §15.1 step 4 has repeated it, but §6.1's ladder searched SSAP unconditionally and decision 69's `Add_Number` gate is silent for want of a query house number — so every address point on the named street was scored, and a rung-1 best at or above the INTERPOLATED_POINT ceiling then ended the search before the road layer was ever read. The street-level answer this section promises was reachable only where the SSAP layer happened to hold nothing on that street. Decision 119 makes rung 1 inapplicable to a query with no usable `Add_Number`, which is what the sentence above always described. Note what did NOT change: nothing is refused, there is still no Gate 1, and the request is still admitted, scored and answered.*
...
***✔ Settled (Session 16, decision 119) — a rung must be APPLICABLE before it can compete.** Rung 1 is applicable only where the query supplies a usable `Add_Number`; where it does not, the SSAP pass yields no candidates and the comparison above begins at rung 2 (itself empty, for want of a number to interpolate) and lands on rung 3. This precedes the confidence comparison rather than participating in it, and it makes the short-circuit above unreachable for a house-number-free query — which was the mechanism by which an address point nobody asked for suppressed the road scan holding the answer that was asked for. See decision 119; the rule lives in §6.2 alongside the other two things resolved before scoring.*
...
***✔ Settled (Session 16, decision 119) — rung APPLICABILITY is not candidate narrowing, and this rule does not reach it.** Where the query supplies no usable `Add_Number` — none sent, or one decision 63 dropped — the SSAP pass returns nothing and the scorer is not called against a single address point. That is not the filter decision 61 abolished and not the administrative gate decision 82 declined: both of those govern which RECORDS may answer a question this rung can answer, and this governs whether the rung can answer the question at all. An address point's identity is its house number — decision 69's own premise — so with no house number in the query every position rung 1 could return asserts an element the caller never supplied and the service has no basis to choose among. Decision 69 and decision 119 are two halves of one rule: 69 governs which records answer a house-number query, 119 governs whether there is a house-number query to answer. §6.2's "discards nothing unseen" property is untouched, because nothing is discarded — no rung-1 candidate set exists to discard from.*

*Two exclusions are not part of this rule and remain in force. §3.4's temporal filter is a correctness test, not a narrowing one — a record outside its Effective/Expire window is wrong rather than merely unlikely, and is excluded before scoring. `GCS_MIN_MATCH_SCORE` (§6.4) applies after every record has been scored, so it discards nothing unseen — this set, everyone who cleared the floor, is what the i3-improved interface returns and what candidate identification hands onward without further narrowing. See decision 118 (§6.3) for the narrower question of which of THOSE survivors get to compete for the i3 interface's single collapsed answer — a separate concern from admission, settled at response assembly rather than here.*
...
***✔ Settled (Session 15, decision 118) — the ambiguity test runs on the TOP-SCORING TIER of the candidate set, not on every candidate that cleared `GCS_MIN_MATCH_SCORE`.** §6.2 admits every record clearing the floor, and until this decision the i3 interface's §6.3 spread test ran across that entire admitted set regardless of how far a candidate trailed the leader. On real data that let a candidate matching only on house number and street name — every discriminating field (`St_Type`, `Community`) zeroed, tens of points below the leader — sit in the same spread computation as two genuinely co-located, fully-matching candidates a few metres apart, and the wide-trailing candidate's distance alone was enough to blow the spread past `GCS_AMBIGUITY_TOLERANCE_M` for a query that was not actually ambiguous. §6.3 now applies only to candidates within a fixed float-tolerance band of the best matchScore in the (already same-rung) admitted set; a candidate that is merely admissible never joins the spread test and never widens or vetoes a real merge among the genuine leaders. The i3-improved interface is unaffected — it was never bound by §6.3's collapse-to-one-answer constraint and continues to return every admitted candidate, trailing ones included, ranked and scored on their own merits (decision 27).*
...
***✔ Settled (Session 16, decision 119) — one new path, joining the list above rather than qualifying it.** A query with no usable `Add_Number` against a street carrying address points but no RoadCenterLine record now yields 468, where it previously returned one of those address points. It is a provisioning defect — RoadCenterLine is Required for the GCS under STA-006.3 Table 4-1, so a street with address points and no centerline should not exist — and returning a house number the caller never supplied is not the better answer to it. Like every other path here it is indistinguishable on the wire, and decision 114's fixed body field does not change that.*
...
*The input’s extent is not discarded. It is carried forward into §10.3 as the containment test, into §10.6 as the damping term on the spatial-fit score, and — per decision 122 — into §12.2 as the narrowing of the reported candidate list to the candidates the shape actually contains, so that a query expressing two kilometres of uncertainty does not score as though it expressed two metres, and a query expressing five metres is not answered with everything inside two hundred and fifty.*
...
***✔ Amended (Session 17, decision 122):** the list is narrowed to the CONTAINED candidates where the input shape has extent and at least one candidate falls inside it — §9's extent-is-the-substance-of-the-question argument reaching the reported set, not only the flag and the damping term. §10.3's order is preserved among the survivors; nothing is re-ranked and no candidate's score changes. Where the shape has extent but contains nothing, the full list is returned unchanged rather than 468 (a placement accident is not the claim that no address exists), and a Point input — zero extent, containing nothing by construction — is unaffected. The strict interface still returns exactly one answer either way (§12.1): the narrowing changes the set rank 1 is elected from, not how many are returned, and provably does not move rank 1 at all, since §10.3 already orders contained candidates ahead of uncontained ones.*
...
GEOCODE(request):
  ...
  4. No Gate 1 exists. A civic address with no Add_Number is admitted
     and answered at rung 3 -- delivered by step 6a below, not by
     this step.                                             (dec 119)
     An Add_Number with no address-level match falls back to rung 3
     rather than failing. Precision degradation is carried by
     uncertainty (§7.4), never by rejection.                 (§5)
  ...
  6. SSAP pass (rung 1):
     a. Applicability, BEFORE the gates and before the scorer is
        called once: if the query supplies no usable Add_Number --
        none sent, or one dropped by decision 63 -- rung 1 yields NO
        candidates and this pass ends here. An address point's
        identity is its house number, so there is no question for
        this rung to answer. Not a refusal: go to step 8.  (dec 119)
     b. Apply the identity gates — these run BEFORE scoring and are
     ...
     c. Score each surviving record (§6.5) -> SCORE_RECORD below.
     d. Disqualify (matchScore forced to 0, breakdown still reported)
        any candidate failing street-name qualification.     (dec 71, 73)

  7. Evaluate whether the RCL pass is needed:               (§6.1, dec 70)
     Unreachable where step 6a ended the SSAP pass -- there is no
     rung-1 candidate to short-circuit on, which is the point of
     decision 119.
...
| GML 3.1.1's schema has no `srsDimension` on `posList`, but real producers put it there | RFC 5491's shapes are expressed in GML, and OGC GML 3.1.1 — the vintage this deployment's bundled schema targets — declares `srsDimension` only on the geometry element (`gml:Polygon`, `gml:LineString`, via `AbstractGeometryType`/`SRSReferenceGroup`), not on `DirectPositionListType`/`gml:posList`. GML 3.2 / ISO 19136 moved the attribute onto the position-list type itself, and that is where real-world `posList` producers commonly place it regardless of which GML major version they nominally target. A caller emitting spec-compliant-for-3.2 `<gml:posList srsDimension="3">` output against a service that (correctly, per 3.1.1) has no attribute declaration for it there receives a 454 — a rejection the caller has no way to anticipate from reading i3 or RFC 5491 alone, since neither standard states which GML vintage's `posList` convention a GCS must accept. Resolved locally (decision 123, Session 18): this deployment's schema now accepts the union of both spellings. | A future revision of i3, RFC 5491, or an accompanying profile should state explicitly which GML version's `posList`/`srsDimension` convention a conformant geocoding service must accept, since 3.1.1 and 3.2 disagree on where the attribute lives and neither i3 nor RFC 5491 resolves the conflict. |
...
### 118 — §6.3, §6.4 (the strict interface's ambiguity/merge test runs on the top-scoring tier of the candidate set, not on every admitted candidate)

**Decision:** `GCS_MIN_MATCH_SCORE` (§6.4) answers exactly one question — is a candidate plausible enough to admit at all — and until this decision `strict_answer` (`src/geocode/response_assembly.py`) also treated "admitted" as "in contention for the i3 interface's single collapsed answer," running §6.3's horizontal-spread test across the FULL admitted set regardless of how far a candidate trailed the leader. That conflated two different questions with one threshold. §6.3's spread test now runs only within the TOP-SCORING TIER of the (already same-rung) admitted set: candidates within `_TOP_SCORE_TIE_EPSILON` (1e-6) of the best matchScore. A candidate that is merely admissible — cleared the floor, but trails the leader by any real editorial margin — no longer joins the spread computation, and can neither widen a real merge's reported extent past `GCS_AMBIGUITY_TOLERANCE_M` nor veto it into a 468. This is scoped to the i3 interface alone: `enhanced_answers` never calls `strict_answer` or `resolve_ambiguity`, so `GeocodeEnhanced` is unaffected and continues returning every admitted candidate, trailing ones included, ranked and scored on their own merits (decision 27) — unchanged since a user could already reason about ambiguity themselves from the full ranked list; the gap was specific to the strict interface's forced single answer.

**Reasoning:** Raised by the user, working from `FWD-AMBIG-MERGE-001`'s regression output: 705 Main Street, Underwood resolves to two real SSAP candidates 3.9 m apart, both scoring 100.0 (every field an exact match) — a textbook §6.3 merge case. But on the full statewide export, two more candidates also clear `GCS_MIN_MATCH_SCORE` for the same query: 705 Main Avenue, Washburn (matchScore 70.92 — `St_Type` and `Community` both zero, `St_Name`/`A2` full credit) and 705 East Main Avenue, Bismarck (66.00, similarly zeroed on the discriminating fields). Neither is a real candidate for this address — both fail on exactly the fields that would distinguish location — but decision 82 deliberately declined an A1/Country/administrative candidate-set gate (a hard state-line or county gate would empty the candidate set for a caller who names the wrong jurisdiction, a live scenario for this deployment), so nothing upstream of §6.3 excludes them. Before this decision, all four candidates entered the same spread computation, measured a 52,693 m span, and failed 468 — for a query where the actual disambiguating information (co-location of the two real units) was never in doubt. The user's proposed fix — restrict the merge/ambiguity test to whichever candidates tie for the top score — was evaluated directly against three alternatives raised in the same discussion:

  1. **Return the top result outright** (no merge machinery at all). Rejected: where the top tier itself has more than one candidate (this fixture's actual case), there is no principled way to pick one over the other without inventing information the query did not supply (here, the unit) — exactly what §6.3's merge exists to avoid asserting.
  2. **Fall back to road interpolation (rung 2/3)** when the point rung looks ambiguous. Rejected: decision 70's comparative ladder already prefers any `ADDRESS_POINT` candidate (confidence ceiling 80) over `INTERPOLATED_POINT` (75) or `STREET_SEGMENT` (50); a road answer would never be selected over the point candidates that already exist here, so it would not have changed this outcome and solves a different problem (no point candidates at all) than the one demonstrated.
  3. **A configurable band width via a new environment variable.** Considered and explicitly declined by the user, on the general ground of environment-variable proliferation rather than any technical objection to a band as such — seven-plus `GCS_*` tuning variables already exist and a mechanism this narrow does not need to join them.

  The adopted design threads that: a fixed, non-configurable epsilon rather than an authored "how close counts as close" judgment. `_TOP_SCORE_TIE_EPSILON` exists solely to absorb float noise, not to encode an editorial band — `src/engine/scoring.py`'s `_weighted_average()` already documents, from a real regression-suite finding, that float summation across several weighted terms is not associative and an all-1.0 match can land a few ulps off 100.0 depending on term order. 1e-6 is many orders of magnitude past that noise floor (the documented artifact was ~1e-14) and many orders of magnitude short of any editorially meaningful score difference (the smallest real gap in this fixture is Washburn's 29-point trail), so it only ever unifies candidates that are the same match by any real-world reading — it never merges two candidates a human would call merely "close." No `.env.example` entry, no `runtime_state.py` binding: this is numerical hygiene in the same register as `_weighted_average()`'s own clamp, not a deployment-tunable constant, and does not belong on the tuning-constants list in `CLAUDE.md`.

**Scope:** Changes `src/geocode/response_assembly.py::strict_answer()` only. `src/geocode/candidates.py::resolve_ambiguity()` is unchanged — it still applies §6.3 to whatever set it is handed; the narrowing happens one layer up, at the caller that owns the strict/enhanced split (module docstring, "THE TWO INTERFACES DIVERGE HERE AND ONLY HERE"). §6.2's candidate SET is unaffected — every admitted candidate is still scored, still ranked, still returned by `GeocodeEnhanced`; only which of them may compete for the i3 interface's single answer changes. §6.4's "no coverage test distinguishes them" invariant (decision 114) is unaffected — this decision changes WHEN 468 fires via the ambiguity path, not what the wire says when it does, and does not introduce a new distinguishable failure mode.

**Verification:** Two `tests/conformance/test_response_assembly.py` tests added — `test_a_trailing_candidate_does_not_join_a_merge_it_would_have_widened_beyond_tolerance` (a two-candidate 100.0/100.0 merge plus a 70.0-scoring, 55 km-distant third candidate now merges cleanly, and the same three candidates handed directly to `resolve_ambiguity` still raise `AmbiguousResult` — confirming the narrowing, not the tolerance or geometry, is what changed) and `test_scores_within_float_noise_of_each_other_still_count_as_tied` (a 1e-9 score gap still merges). One existing test (`test_a_merge_makes_the_two_interfaces_return_different_geometry`) used two differently-scored candidates (95.0/90.0) to exercise unrelated geometry-divergence behavior; changed to two tied candidates (95.0/95.0) so it continues to isolate what it was written to test rather than incidentally exercising this decision's narrowing. Full `pytest` suite: 523 passed (521 + 2 new), 5 skipped. `tests/regression/`: 36/38 on first run after the code change, with exactly the two predicted fixtures failing — `FWD-AMBIG-MERGE-001` (468 → 200, a Circle centred on the two real Underwood units, radius 1.97 m, confidence 80.0) and `FWD-UNIT-DISAMBIG-001` (468 → 200, a single Point, confidence 80.0, no synthesised uncertainty per decision 57) — reviewed before reseeding per this suite's own "seed once, never rebuild" discipline; `FWD-AMBIG-CANDIDATES-ENH-001` (the enhanced sibling over the same address) passed unchanged throughout, confirming the enhanced interface's scope exemption held in practice. 38/38 after reseeding both golden files with `seed.py --force`.

### 119 — §3.3, §5, §6.1, §6.2, §6.4 (rung 1 is inapplicable to a query that supplies no usable house number; the rung-3 answer §5 has always promised is now what the ladder actually produces)

**Decision:** The SSAP pass (rung 1) yields no candidates at all — and the scorer is not called against a single address point — where the query supplies no usable `Add_Number`. Two ways in: the caller sent no `ca:HNO`, or sent one decision 63 dropped for not reducing to a non-negative integer. The ladder then begins at rung 2, which is itself empty for want of a number to interpolate, and lands on rung 3. §3.3's ladder table is corrected to state the condition on rung 1's own row, where rungs 2 and 3 already carried theirs.

This is an APPLICABILITY test for a rung, not a filter over a candidate set, and it is neither of the two things it resembles. Decision 61 abolished the progressive filter and decision 82 declined an administrative candidate-set gate; both govern which RECORDS may answer a question the rung is capable of answering. This governs whether the rung is capable of answering the question at all. An address point's identity IS its house number — that is decision 69's own premise, and the reason `Add_Number` is a gate rather than a scored term — so a query with no house number has nothing for rung 1 to be an answer to. Decision 69 and this decision are two halves of one rule: 69 governs which records answer a house-number query, 119 governs whether there is a house-number query to answer.

Nor is it Gate 1 (§5, decision 14), which would REFUSE the request. Nothing is refused: the request is admitted, scored against the road layer, and answered — at rung 3, which is what §5, decision 63 and §15.1 step 4 have all said since Session 1.

**Reasoning:** Raised by the user, working from `FWD-DROPPED-HNO-ENH-001`'s regression output, and confirmed as a code-versus-specification disagreement rather than a new policy question: this document asserted the rung-3 outcome in three places and its own mechanism could not produce it.

The fixture queries `A2=Burleigh County, A3=Bismarck, cae:STP=Interstate, RD=94, STS=Ramp` with an `HNO` decision 63 drops. It returned two SiteStructureAddressPoint candidates — 13100 and 13101 Interstate 94, matchScore 96.733, confidence 77.386 — to a caller who supplied no house number at all. Neither record is a ramp (`St_PosTyp` empty; the query's `Ramp` scored 10.0 against `Interstate`, a term whose weight is too small to move a four-term average), and neither is in the city of Bismarck: both carry an empty `A3`, and their "Bismarck" comes only from `Post_Comm`, the postal area, which extends well past the municipal boundary. Because 77.386 exceeds the INTERPOLATED_POINT ceiling of 75, §6.1's short-circuit then ended the search **before the RoadCenterLine layer was scored at all**. Scoring it directly shows what was suppressed: 19 segments tied at matchScore 100.0, every one of them `St_PreTyp=Interstate, St_Name=94, St_PosTyp=Ramp, A3_L=A3_R=Bismarck, A2=Burleigh County` — an exact match on every element the caller supplied, including the two the address points failed, and inside the city as a real municipal value rather than a postal one. The service had the right answer in a Required layer and never looked at it.

Three things had to be true at once, which is why this survived to Session 16 with the suite green. Decision 69's `Add_Number` gate is conditional on the query supplying one, so it is silent on exactly the queries this decision concerns. Decision 71's street-name gate zeroed every unrelated address point, leaving these two as the only rung-1 survivors and therefore an unambiguous-looking answer. And decision 70's short-circuit is arithmetically sound on its own terms — no road answer CAN out-score an ADDRESS_POINT candidate, because the ceilings price precision — but the ceilings price precision, not correctness, and the case they cannot see is a rung-1 candidate that is a worse match to the same query. The repair belongs upstream of the comparison, not in it: with no rung-1 candidates the short-circuit is simply unreachable, and decision 70 needs no amendment.

The conformance suite encoded the contradiction rather than catching it. `test_a_query_with_no_house_number_is_accepted_at_rung_3` provisioned no SSAP records at all, so it passed for the wrong reason; `test_a_query_with_no_house_number_gates_nothing` asserted the opposite behaviour outright, reading decision 61's "no filter" rule as covering a case decision 61 was not addressing.

Two adjacent findings were identified in the same investigation and are deliberately NOT folded in here, each being a separate question with a separate answer:

  1. **The Community cascade compares across tiers without penalty.** Decision 76 resolves each side independently (A3 → A4 → `Post_Comm`), so a query asserting `A3="Bismarck"` scores 100 against a record whose only "Bismarck" is postal. Penalising the cross-tier case was considered and rejected on decision 80's own reasoning: a caller saying "Bismarck" for a rural Burleigh County address with a Bismarck mailing address is the normal case rather than an error, 21.8% of SSAP records resolve Community from `Post_Comm`, and a penalty would cost far more true matches than false ones. It is also not what produced this defect — once the rung is correct, the winning candidates carry `A3=Bismarck` as a real municipal value and the symptom is gone. No change.
  2. **Rung 3 has no ambiguity test.** `strict_answer` returns `candidates[0]` for a line answer, so the i3 interface here picks one of 19 segments tied at 100.0 and spread ~7 km along the corridor, chosen by NGUID sort order and reported without qualification. That is the hazard §6.3 exists for, on the one rung §6.3 explicitly does not reach ("a rung-3 set has no positions to average"). Decision 118 settled the tier question for positioned candidates; the same question for segments has not been asked. Left open deliberately — it needs its own decision about what an i3 single-answer response should say when the honest answer is a set of segments, and answering it inside this decision would bundle two unrelated changes into one reseed.

**Scope:** `src/geocode/candidates.py::ssap_candidates()` only — an early return before the record loop, with decision 69's per-record gate becoming unconditional inside it (the early return now guarantees the query has a number to compare). `identify()`, `rcl_candidates()`, `resolve_ambiguity()`, `src/geocode/response_assembly.py` and the scorer are all unchanged. The reverse direction is untouched: it carries no house number by construction (§11.2) and has no ladder. Both interfaces change together — unlike decision 118, this is a candidate-identification rule, not a strict-interface collapse rule, so `GeocodeEnhanced` returns the rung-3 candidate list for these queries just as `Geocode` returns a rung-3 answer. A query supplying a house number is bit-for-bit unaffected, which is the common case and every fixture that supplies one.

**Verification:** Full `pytest`: 524 passed, 5 skipped. Two tests in `tests/conformance/test_candidates.py` carry the decision — `test_a_query_with_no_house_number_never_reaches_rung_1_at_all` (asserts both the empty result AND that the injected scorer was never called, since "returned nothing" and "scored nothing" are different claims and only the second shows the rung was skipped rather than emptied) and `test_rung_1_being_inapplicable_is_not_a_gate_1_refusal` (the same query still answered, at rung 3, from the road layer). `test_a_query_with_no_house_number_is_accepted_at_rung_3` now provisions SSAP records on the queried street, which is what it should always have done — it previously passed on an empty SSAP layer and would not have caught this. `test_a_query_with_no_house_number_gates_nothing` is removed, its claim being the defect. Fourteen further tests in that file used a house-number-free query as incidental setup while testing decision 61's no-filter rule, the `GCS_MIN_MATCH_SCORE` floor, §3.4 temporal exclusion, §6.3 merging and §7.4 ranking; each gained a matching `Add_Number` on the query and its fixtures so it continues to test what it was written to test. One of them, `test_ranking_is_deterministic_across_calls`, was asserting `run() == run()` and would have gone on passing vacuously on two empty lists — it gained a length assertion first. `tests/regression/`: `FWD-DROPPED-HNO-ENH-001` and `FWD-DROPPED-HNO-STRICT-001` change as predicted and are reseeded after review; every other fixture passes unchanged, all of them supplying a house number.

### 120 — Appendix C.2 item 9 (partial) — rung-3 ties are broken by proximity to the query's house number where one exists, narrowing but not closing item 9

**Decision:** Where the query supplies a usable `Add_Number` and rung 3 (whole-segment) candidates tie at the top confidence/matchScore, the tie is now broken by how close each candidate's own asserted address range sits to that `Add_Number` — the segment whose range comes nearest wins — before falling back to item 9's existing NGUID tiebreak for any remainder. Where the query supplies no usable `Add_Number`, behaviour is unchanged: `_rank`'s NGUID tiebreak still applies, exactly as item 9 describes. `src/geocode/candidates.py::_rank_segments()` implements this; `rcl_candidates()` calls it for the rung-3 list only, in place of `_rank`.

**Reasoning:** Raised by the user from `FWD-GAP-HNO-001`'s regression output: a query for 1911 Calgary Avenue — a house number falling in a genuine gap between two RoadCenterLine ranges (`...1901-1909` and `1917-1999...`, nothing covers 1911-1915) — returned a segment numbered in the 3000s, nowhere near either bracketing range. Tracing it found the mechanism item 9 already named, reached from a different door: rung 3's matchScore has no house-number term (interpolation already failed, by construction, for every rung-3 candidate), so once a query's street name matches with no other disambiguating civic element, every same-named segment city-wide ties at identical confidence — not just the two segments bracketing the gap. Against `data/data.gpkg`, this query ties **46 Calgary Avenue segments** at matchScore 100.0, from the 100 block to the 5000 block, and `_rank`'s NGUID tiebreak happened to put `{076D8E0E-...}` (3000-3298) first — an alphabetic accident of the GUID text, unrelated to the corridor.

Item 9 was written from `FWD-DROPPED-HNO-STRICT-001`'s shape — no house number supplied at all — where there is nothing to measure a gap against and the honest answer may genuinely be "one of several distant, tied segments," which is why item 9 reaches for a 468/spread-test/MultiLineString answer. `FWD-GAP-HNO-001`'s shape is different: the caller DID supply a house number, and while no segment's range contains it, the segments do carry ranges the query's number can be compared to. Preferring the nearer one is not a new scoring signal or a new field weight — decision 89 and the withheld §6.5 formula are untouched, and matchScore does not change — it is a tiebreak among already-equal-scoring candidates, in the same register as `_rank`'s existing NGUID tiebreak (§10.4, decision "(e)" in Appendix C.2's closing note), just a more geographically meaningful one where the information to make it exists.

**Scope:** `src/geocode/candidates.py` only — `_range_gap()` (new) and `_rank_segments()` (new), reusing `position.side_ranges()` rather than re-deriving range logic; `rcl_candidates()`'s return statement calls `_rank_segments(segments, query.Add_Number)` in place of `_rank(segments)`. `_rank` itself, rung 1 (SSAP), and rung 2 (interpolated) are unchanged — deliberately: a rung-1 tie already shares one `Add_Number` by construction (decision 69), and a rung-2 tie's candidates already contain the query's number by construction (that is why they interpolated), so a range-proximity tiebreak has no purchase on either and was not added to them. `identify()`, `resolve_ambiguity()`, `response_assembly.py`, and the scorer are all unchanged. A query supplying no `Add_Number` is bit-for-bit unaffected (`_rank_segments` reduces to `_rank` exactly in that case) — `FWD-DROPPED-HNO-STRICT-001` and `FWD-DROPPED-HNO-ENH-001` are unchanged and reconfirmed below.

**What this does not resolve:** Item 9 stays open. This decision narrows it to the case it can actually address — a house number that fails to land in any range but that ranges exist to compare against — and does nothing for the no-house-number case, where there is no query number to break the tie with and item 9's harder question (468? a bounding shape? an explicit arbitrary-but-deterministic finding?) is unanswered. Nor does it address whether 46 (or 19) tied segments, even correctly ordered, should ever collapse to a single silent i3 answer at all — the tiebreak picks a better *first* candidate, it does not add the ambiguity test §6.3 does not reach. Appendix C.2 item 9 is amended, not closed, to say so.

**Verification:** Two `tests/conformance/test_candidates.py` tests added — `test_rung_3_ties_are_broken_by_proximity_to_the_queried_house_number` (two tied segments with the FARTHER one given the alphabetically-first NGUID, so the assertion only passes if the proximity tiebreak actually ran, not by NGUID coincidence) and `test_rung_3_ties_fall_back_to_nguid_when_the_query_has_no_house_number` (the no-`Add_Number` shape confirmed unchanged). Full `pytest`: 526 passed (524 + 2 new), 5 skipped. `tests/regression/`: 37/38 on first run after the code change, with exactly the predicted fixture failing — `FWD-GAP-HNO-001` (3000-3298 block → the 1901-1909 block bracketing the low side of the gap, the nearer of the two by 2 vs. 6 house numbers) — reviewed and reseeded with `seed.py --force`. `FWD-DROPPED-HNO-STRICT-001` and `FWD-DROPPED-HNO-ENH-001` (no `Add_Number` in the query) passed unchanged, confirming the no-house-number path is untouched. 38/38 after reseeding.

### 121 — Appendix C.2 item 9 (closes) — §6.3's ambiguity test is extended to rung 3, narrowed by `_rank_segments`'s own key rather than by matchScore alone

**Decision:** `resolve_segment_ambiguity()` (`src/geocode/candidates.py`) extends §6.3's spread-and-refuse test to rung 3. It measures the greatest pairwise geodesic distance between the top-scoring tier's own segment midpoints (`_segment_spread_m()`, using `geometry.midpoint()` — §3.7.3's minimise-maximum-error point, the same rationale decision 48 already uses for a single segment's own answer position) and raises `AmbiguousResult` — the same exception, and the same `GCS_AMBIGUITY_TOLERANCE_M` decision 54 introduced, not a second tolerance knob — when that spread exceeds tolerance. It cannot merge (§7.4, decision 30: there is no average of two line segments that means anything), so it can only return the ranked leader or refuse. `response_assembly.strict_answer()` gains a `house_number: Optional[int]` parameter, threaded from `conversion.query.Add_Number` via `api/conversion.py::strict_forward_answer()`, and uses it to narrow the rung-3 tier on the *same key* `_rank_segments` ranks by — matchScore first (decision 118's existing `_TOP_SCORE_TIE_EPSILON`), then, only where a house number was supplied, `range_gap` within that tier — before handing the result to `resolve_segment_ambiguity()`. `_range_gap` is renamed `range_gap` (module-public), since it is now a cross-module dependency rather than a private helper local to one tiebreak.

**Reasoning:** This closes item 9's remaining half — no house number in the query at all (`FWD-DROPPED-HNO-STRICT-001`), where `_rank_segments` reduces to `_rank` and every same-named segment ties with no geographic meaning to the NGUID sort that used to decide among them. Before this decision that query silently returned one of 19 real segments spread ~7 km along the I-94 corridor as if it were a confident single answer, at confidence 50.0 with nothing marking it as one of nineteen — exactly §6.3's own "two State Street matches forty miles apart are not a location" hazard, on the one rung §6.3 did not reach.

The tier the spread test runs on had to narrow on the *same* key `_rank_segments` ranks by, not on matchScore alone — a matchScore-only version was caught by regression before it shipped. `FWD-GAP-HNO-001` (decision 120's own fixture — HNO 1911, `range_gap` already narrowing the true ordering to a single genuine segment, the 1901-1909 block) sits in the *same* wide matchScore tie as every other same-named segment on that street, for the identical reason above: no matchScore term sees `range_gap` at all. A matchScore-only tier therefore caught both fixtures at once and would have silently reopened decision 120's already-settled pick. The corrected mechanism narrows twice: matchScore first, then — only where the query supplies a house number — `range_gap` within that tier. Where `range_gap` already distinguishes a unique leader, the spread test never runs and decision 120's pick stands untouched. Where it does not — no house number at all, or a genuine tie in `range_gap` itself (an HNO sitting exactly on the boundary between two ranges kilometres apart, each range legitimately claiming it) — the wider tier reaches the spread test, which is the correct outcome in both cases: the first is item 9's original gap; the second is a case neither "HNO present" nor "HNO absent" alone would have caught, caught here for free because the narrowing is on information, not on presence.

`resolve_segment_ambiguity()` itself takes no house number and stays agnostic to why its input tier was already narrowed — the division is deliberate, so the spread test is one reusable "do these still-tied leaders actually agree" primitive regardless of which signal did the narrowing above it.

**Scope:** `src/geocode/candidates.py` (`_segment_spread_m`, `resolve_segment_ambiguity`, new; `_range_gap` → `range_gap`, renamed), `src/geocode/response_assembly.py` (`strict_answer()` gains `house_number`, rung-3 branch narrows before calling `resolve_segment_ambiguity`), `src/api/conversion.py` (`strict_forward_answer()` threads `conversion.query.Add_Number` through). `resolve_ambiguity()`, `_rank`, `src/api/geocode.py`, `src/api/enhanced.py`, and `GCS_AMBIGUITY_TOLERANCE_M`'s config/default handling are all unchanged — the existing `AmbiguousResult`-to-468 mapping in `api/geocode.py` covers this case with no new wiring. `enhanced_answers()` is unaffected: it never calls `strict_answer` or `resolve_segment_ambiguity`, so `GeocodeEnhanced` continues to return every tied candidate, ranked, on both fixtures.

**Verification:** `FWD-GAP-HNO-001` is a MUST-PASS regression, not a reseed — it returns to its decision-120 golden (200, single LineString, 1901-1909 block) unchanged. `FWD-DROPPED-HNO-STRICT-001` reseeded: 468 (19 candidates, 6932.2 m spread, beyond `GCS_AMBIGUITY_TOLERANCE_M`), replacing the prior silent single-LineString 200. A new synthetic fixture covers the boundary case — two segments tied on `range_gap` (both 0, or within epsilon) several kilometres apart with a house number supplied — confirmed 468. Tests added in `test_candidates.py`, `test_response_assembly.py`, and `test_conversion_http.py` (468 at the HTTP layer, and `GeocodeEnhanced` confirmed unaffected on the same inputs). Full `pytest`: 540 passed, 5 skipped. `tests/regression/`: 38/38, including `FWD-GAP-HNO-001` passing untouched.

### 122 — §9, §10.3, §12.2 (the reverse candidate list narrows to the contained candidates where the input shape has extent)

**Decision:** Where the reverse input shape has extent and at least one candidate falls inside it, the candidate list is narrowed to the contained candidates. `answers()` (`src/reverse/response_assembly.py`) applies this after building §10.3's ordered list and before returning it, using the `SearchOrigin.has_extent` and `ReverseAnswer.contained` values that already existed. No new field, no new configuration variable, and no change to `GCS_REVERSE_SEARCH_RADIUS_M`, which keeps its current meaning and role.

**The extent is the substance of the question, not metadata about it.** §9 says so directly: on the Geocode side an input's uncertainty is metadata attached to a civic address and properly ignored (§7.4), whereas here "the input geometry is the query itself, so its extent is not metadata about the question but the substance of it." Until this decision the code honoured that statement in only two places — §10.3's `contained` flag and §10.6's extent-damping term — while the candidate SET was bounded by the flat `GCS_REVERSE_SEARCH_RADIUS_M` regardless of shape. A 5 m circle and a bare `gml:Point` therefore retrieved an identical list. On the provisioned Bismarck data, `REV-CONTAINED-POLYGON-ENH-001`'s small polygon returned 154 candidates out to 250 m, of which 54 were actually inside the polygon the caller asserted the target lies within; `REV-CIRCLE-ENH-001`'s 100 m circle returned the same 154, of which 49 were inside it. A caller who states where the target is and receives a hundred addresses demonstrably outside that statement has been answered a question they did not ask.

**This is assembly-time narrowing, not a search bound and not a pre-scoring filter**, and the distinction is load-bearing. Bounding the search by the shape was rejected because §10.1 specifies one pass with no separate containment pass — both containment and the 3D test are expressed as terms in the ordering rather than as gates in front of it, on decision 28's reasoning that a distinction expressible as a position on one continuous ordering should not be reimplemented as a second code path that can readmit what the first rejected. Filtering in front of the scorer was rejected for the reason decision 61 removed exactly that shape of filter on the forward side: anything a pre-scoring filter excludes is excluded permanently, so a filtered candidate is one where a data defect produces a 468 rather than a low score. Narrowing at assembly keeps the single pass intact, keeps every candidate scored, and confines the change to which of the scored candidates are reported — which is the same place and the same reasoning as decision 118, where the forward strict answer narrows to the top-scoring tier after §6 has scored everything.

**Nothing contained falls back to the full list, and that is not a 468.** A tight shape over empty ground — a 5 m circle in a field, a building footprint whose address point sits a metre outside it through ordinary placement error — degrades to today's nearest-within-radius behaviour. Refusing there would convert a data-placement accident into the claim "no address exists," which is a materially different and worse statement than the approximate answer §10.5 already discloses the raw distance for. 468 keeps its existing single meaning: nothing fell within `GCS_REVERSE_SEARCH_RADIUS_M`. `REV-NOMATCH-OPENWATER-001` is unaffected.

**A Point input is bit-for-bit unaffected.** Its extent is 0, so `has_extent` is False and the branch never runs; and a Point contains nothing by construction (§10.3), so there would be nothing to narrow to even if it did. The commonest input in the system takes exactly the path it always has — `REV-POINT-SSAP-ENH-001` (154 candidates) and `REV-RCL-SYNTH-HNO-ENH-001` (2) are unchanged, and are pinned as such rather than reseeded. `REV-CROSSCOUNTY-001` is likewise unchanged for the same reason: it carries a Point, not a shape with extent.

**The strict interface still returns exactly one answer (§12.1).** This decision changes which candidates the enhanced list reports (§12.2) and the set rank 1 is elected from; it does not change how many answers the i3 interface returns, and it does not change §10.6's scoring — a narrowed list still reports the same damped confidence per candidate.

It does not, in fact, change the elected answer either, and that is worth recording rather than leaving to be rediscovered. §10.3's `ordering_key` already sorts contained candidates ahead of uncontained ones, so the contained answers were always a prefix of the ordered list and rank 1 was ALREADY the contained candidate wherever one existed. The narrowing removes only candidates that were ranked below the strict answer. The three strict reverse fixtures carrying a shape with extent — `REV-POLYGON-001`, `REV-ELLIPSE-001`, and `REV-SPHERE-VALID-001` — pass unchanged, and a conformance test (`test_the_narrowing_does_not_move_rank_1`) pins the property directly so a future re-sort of the reverse ordering cannot quietly make this decision start changing the i3 answer.

**Shape coverage, stated because it is not yet fully tested.** The narrowing is shape-agnostic by construction: it keys only off `has_extent` and `contained`, with no shape-specific branch, so all seven extent-bearing forms in `src/reverse/origin.py` (Circle, Sphere, Ellipse, Ellipsoid, ArcBand, Polygon, Prism) inherit it and Point is excluded by its zero extent. Only Circle and Polygon are exercised by fixtures as of this decision; enhanced fixtures for the remaining five are outstanding work, not a behavioural question. ArcBand is the one shape whose behaviour under this decision is counterintuitive and should be pinned deliberately: it is an annulus, so its `contains()` excludes the hole, and narrowing can therefore drop a candidate NEARER the origin than ones it keeps. That is correct — the caller asserted the target lies in the band, not in the middle — and it follows from the same reasoning §10.6 already uses to refuse weakening containment for large shapes.

**Verification:** `REV-CONTAINED-POLYGON-ENH-001` 154 → 54 and `REV-CIRCLE-ENH-001` 154 → 49 (max surviving distance 98.6 m against a 100 m radius), both reseeded after confirming every survivor carries `contained=True`. Point-input enhanced fixtures and every strict reverse fixture unchanged and pinned rather than reseeded. New unit tests cover the nothing-contained fallback returning the full list (not empty, not 468) and the contained case preserving relative order. Full `pytest`: 544 passed, 5 skipped. `tests/regression/`: 38/38.

### 123 — `schemas/gml-geoshape/geometryBasic0d1d.xsd` (`DirectPositionListType` gains `srsDimension`, admitting a `posList`-level spelling the parser already read)

**Decision:** `DirectPositionListType` — the type behind `gml:posList` — now declares an optional `srsDimension` attribute, alongside its existing `count`. This is a deliberate, documented extension beyond stock OGC GML 3.1.1 — the file's own `version="3.1.1"` header is otherwise an unmodified copy — not a correction of a typo in the bundled file and not a silent backdating to GML 3.2.

**Stock 3.1.1 declares `srsDimension` exactly once, on `AbstractGeometryType` via `SRSReferenceGroup`: it belongs to the GEOMETRY ELEMENT** (`gml:Polygon`, `gml:LineString`, and so on), not to the coordinate list. GML 3.2 / ISO 19136 moved it onto the position-list type itself, and that is where real-world `posList` producers commonly put it in practice — `<gml:posList srsDimension="3">...</gml:posList>`, dimensionality declared on the coordinate list rather than (or in addition to) the geometry wrapping it. `src/api/wire/gml_xml.py::_ring_vertices` has read `srsDimension` directly off the `posList` element on exactly that convention since before this decision (falling back to 2 where absent) — the parser was already correct for the input it needs to accept. The schema was the gap: a 3D `posList` this project's own parser reads without complaint 454'd at admission, before `parse_shape()` ever ran, because `DirectPositionListType` had no attribute declaration to validate `srsDimension` against.

**Found building `REV-PRISM-ENH-001`** (decision 122's Prism regression fixture): the natural GML rendering of a 3D polygon ring — one `posList` carrying `srsDimension="3"` — could not pass schema validation, forcing that fixture onto the `gml:pos`-per-vertex spelling instead (also legal GML, also already read by `_ring_vertices`, and it stays on that spelling — this decision does not touch it, since `REV-PRISM-ENH-001` is decision 122's coverage and should keep testing the form it already tests). `tests/conformance/test_wire_gml.py`'s `PRISM` constant used the `srsDimension="3"` form from the start and never caught this, because that test calls `parse_shape()` directly and bypasses schema validation entirely — a real, if narrow, illustration of why this suite's own discipline insists on confirming an observed outcome against the real, fully-validating path rather than trusting a parser-only unit test to stand in for one.

**Schema-side, not parser-side, because the parser was not the defect.** `_ring_vertices` already implements the correct, permissive reading; changing it to match the schema's restriction would have meant deleting a capability the code depends on (this exact fixture) rather than fixing an oversight. The fix accepts the UNION of both spellings — dimension declared on the geometry element per 3.1.1, or on `posList` itself per 3.2-and-common-practice — rather than picking one and rejecting the other. `AbstractGeometryType` / `SRSReferenceGroup` are untouched, so the 3.1.1 spelling still validates exactly as it always did; nothing that passed before stops passing. `gcs-pidflo.xsd` and `geometryBasic2d.xsd` were traced and confirmed to hold no independent or duplicate `DirectPositionListType` declaration — the type is declared exactly once, so one edit is sufficient.

**Two independent lines of defense remain distinct on purpose.** `positiveInteger` is a real type constraint — a non-numeric `srsDimension` still 454s at schema validation, never reaching the parser — but is not narrowed to `{2, 3}` at the XSD level, so `gml_xml.py`'s own runtime check (`dimension not in (2, 3)` → `GmlParseError`) is still the actual, necessary guard against a schema-valid but semantically-unsupported value like `srsDimension="5"`. Confirmed by test, not assumed: a schema widening that quietly subsumed the parser's own guard would have been exactly the kind of regression worth catching here.

**§16 gap register.** This belongs there as well as in this appendix. Appendix B records an internal implementation decision; §16 exists specifically to record where this service's wire behavior diverges from a strict reading of a referenced standard, for a NENA audience integrating against it externally. This is exactly that shape: an external client generating RFC 5491/GML 3.2-style `posList srsDimension` output — plausible, spec-compliant-in-3.2 behavior from a caller with no reason to know this schema is pinned to 3.1.1 — used to get a 454 it would NOT get against a strict OGC GML 3.1.1 validator that simply does not recognize the attribute. Before this decision, this service was accidentally stricter than baseline 3.1.1 in this one specific case — the attribute existing but unrecognized, not a genuine malformation — and that accidental strictness is exactly what §16 exists to surface for an integrator before they find it by trial and error. See the corresponding §16 row.

**Verification:** Four new tests in `test_wire_gml.py`: the schema now admits `posList srsDimension="3"` end-to-end (validated and parsed correctly, the same Prism shape `REV-PRISM-ENH-001` uses); the ordinary 2D case (implicit and explicit `srsDimension="2"`) still validates; the schema still rejects a non-numeric `srsDimension` (454 at admission, never reaching the parser); and `gml_xml.py`'s own `dimension not in (2, 3)` guard still fires for a schema-valid-but-semantically-unsupported `srsDimension="5"`, confirming the widened schema did not quietly remove the parser's own line of defense. Full `pytest`: 557 passed, 5 skipped. `tests/regression/`: 43/43, zero diffs — `REV-PRISM-ENH-001` untouched, as it should be.

### 124 — Appendix C.2 item 10 (closes) — a co-located, cross-HNO reverse tie is a data placement defect, not an algorithmic ambiguity; election stays as-is

**Decision:** No code changes. §10.4's existing NGUID tiebreak stands, unmodified, for the case item 10 raised — several reverse candidates carrying *different* house numbers tied at identical containment, tier, and zero distance because their SSAP records share one coordinate. The item is closed on the finding that this condition is a data placement defect to be corrected upstream, in provisioning, not a scoring or response-shape question this document should build a mechanism to paper over.

**The legitimate multi-unit case item 10 worried this might be confused with does not, in fact, reach the tie condition at all.** Checked against the provisioned Bismarck data (the 1780 East Calgary Avenue, Units 1–6 example already on record from this session): candidates sharing one house number but differing by unit each carry their own distinct point geometry — `placementMethod: "Parcel"`, offset from one another by ordinary parcel-derived placement — not a single shared coordinate. GCS does not synthesize per-unit offsets; STA-006.3's SSAP model places each unit's point independently in the source data. §10.5's ordinary distance ordering therefore already resolves the multi-unit case correctly and unambiguously, with no need for a merge, a spread test, or any new mechanism — item 10's premise that this needed handling was mistaken, not merely unaddressed.

**What item 10 actually found — four SiteStructureAddressPoints at differing house numbers (1760, 1750, 1740, 1710 East Calgary Avenue) sharing one identical coordinate — is therefore not the multi-unit case at all, and is not a shape a correctly-provisioned SSAP layer produces.** Distinct civic addresses sharing one point is a placement error: the points should be moved to their own coordinates, the same conclusion this document has reached before for other data-quality findings (the six-county metro artifact, Session 9; the near-uniform A1/Country field, Q30) — measure against real data, and where the defect is in the data rather than the algorithm, say so and fix it there rather than building compensating logic into the scorer.

**No diagnostic is added.** A cheap warning at the NGUID-tiebreak site was considered and declined: this condition is rare enough, and specific enough to a data-placement mistake, that it is better caught and fixed in the GIS provisioning pipeline than surfaced as runtime noise in a service that has no way to act on it.
...
| 9      | **CLOSED — decision 120, decision 121 (raised Session 16, decision 119; narrowed Session 17 decision 120; closed Session 17 decision 121).** Rung 3 had no ambiguity test, so the i3 interface's single-answer constraint resolved a tie among segments by picking one silently — `strict_answer` returned `candidates[0]` where no candidate carried a position, on the true ground that "a rung-3 set has no positions to average and averaging segment geometries is not a thing §7.4 does," which is a reason the §6.3 merge cannot apply, not a reason no test should exist. Decision 120 closed the house-number-present shape (`FWD-GAP-HNO-001`, 46 same-named segments tied at matchScore 100.0, broken by proximity of each segment's own address range to the query's `Add_Number`). Decision 121 closes the remaining shape — no house number to measure proximity against at all (`FWD-DROPPED-HNO-STRICT-001`, 19 segments tied at matchScore 100.0, spread ~7 km along the I-94 corridor) — by applying §6.3's spread-and-refuse test directly to segment geometries (`resolve_segment_ambiguity`, `_segment_spread_m`), narrowed to the tier `_rank_segments` itself cannot already distinguish (matchScore, then `range_gap` where a house number is available) so decision 120's already-settled picks are never reopened. This also resolves, for free, the boundary case neither shape alone covers: an `Add_Number` tied on `range_gap` across segments kilometres apart. Both shapes, and that boundary case, now resolve to either a single trustworthy answer or an honest 468 — never a silent, arbitrary pick. The i3-improved interface is unaffected — `GeocodeEnhanced` still returns every candidate, ranked and scored, and a consumer can see the tie. | 6.3, 7.4, 8.1 |

| 10     | **CLOSED — decision 124 (raised Session 17, alongside decision 122; closed Session 18).** The reverse strict interface elects one answer from a set of contained, co-located, tied candidates with no signal that it did so — raised on the provisioned Bismarck data, where four SiteStructureAddressPoints (HNO 1760, 1750, 1740, 1710 East Calgary Avenue) sit at the identical coordinate and tie on containment, tier, and distance alike, with §10.4's NGUID tiebreak electing one arbitrarily. Closed on the finding that this is a data placement defect, not an algorithmic ambiguity §6.3's merge, decision 121's spread test, or any new mechanism should compensate for: the legitimate case this might have been confused with — several units of one address sharing a coordinate — does not in fact occur in correctly-provisioned data, since STA-006.3's SSAP model places each unit's point independently, and §10.5's ordinary distance ordering already resolves that case without ambiguity. Distinct civic addresses (different house numbers) sharing one coordinate is a placement mistake to be corrected upstream, the same posture this document has taken toward other data-quality findings (the six-county metro artifact, Session 9). No diagnostic was added; election stays exactly as it was. | 10.4, 12.1 |

**C.2 was reopened by item 10 (Session 17) and is empty again as of Session 18.** It was emptied at Session 11 and stayed so through decisions 107–118; decision 119 reopened it with item 9, deliberately recording the rung-3 tie case as an open question rather than answering it while fixing an adjacent defect. Decisions 120 and 121 (Session 17) closed item 9 in two halves. Item 10 was raised later in the same session, from the reverse side, while implementing decision 122 — recorded rather than answered on the spot for the same reason item 9 was, since the mechanism it looked like it might need was not the mechanism decision 122 was built with. Decision 124 (Session 18) closes it on a different footing than expected: not by building the mechanism, but by checking the premise against real data and finding the case that looked like it needed one does not occur, and the case that does occur is a data defect outside this document's remit to compensate for algorithmically. Item 10 is the only item C.2 has held since decision 121, so its closure empties the section again.
...
**Two small implementation follow-ups from Session 17, one closed the same session, one still outstanding.** Enhanced-interface regression fixtures for the five reverse input shapes that previously had none — Ellipse, Sphere, Ellipsoid, Prism, and ArcBand — are now in place (`REV-ELLIPSE-ENH-001`, `REV-SPHERE-ENH-001`, `REV-ELLIPSOID-ENH-001`, `REV-PRISM-ENH-001`, `REV-ARCBAND-ENH-001`), each verified against real hit data before seeding via a new build-time check (`_narrowing_demonstrated`) that refuses to write a fixture whose narrowed count isn't strictly between zero and the unnarrowed count. Sphere/Ellipsoid confirmed matching Circle/Ellipse exactly under the documented no-altitude fallback (§10.5); Prism confirmed matching Polygon exactly on the same footprint; ArcBand's `extent_m` formula (`max(abs(r2 - reach), abs(reach - r1))`) was hand-verified against real parameters with no defect found, and its hole-exclusion behavior — the nearest candidate to the origin, including the exact address point, falling in the hole and being excluded — was independently confirmed against raw distances rather than trusted from the fixture comment.

~~Second~~: the two rank-1 election sites on the reverse side (`src/api/reverse_geocode.py` reading `converted.answers[0]` directly, `response_assembly.strict_answer()` performing the identical election for library and test callers) are **closed**. `ReverseConversion` gained an `answer` property (`self.answers[0] if self.answers else None`, mirroring `.found`); the route now reads `converted.answer` instead of indexing the list itself. `strict_answer()`'s signature and body are otherwise unchanged — a shared helper was considered and declined, since two independently-testable one-line elections were judged not to need the cross-module coupling. The docstrings on both are corrected to state plainly that `strict_answer()` is library/test-facing only and not on the HTTP path, which was the actual defect: the prior wording implied it was the request-path election. A new conformance test drives the real HTTP route and independently rebuilds the origin/hits to call `strict_answer()` directly, asserting the two elect the same candidate — proving the property rather than asserting it.

**One item from Session 17 surfaced while adding the shape fixtures is now resolved by decision 123.** Building `REV-PRISM-ENH-001` with a 3D `gml:posList` (the `srsDimension="3"` spelling `gml_xml.py::_ring_vertices` already parsed) produced a 454 at schema validation — `schemas/gml-geoshape/geometryBasic0d1d.xsd`'s `DirectPositionListType` declared only `count`, with no `srsDimension` attribute anywhere in the bundled schema, even though the parser read it without complaint. `tests/conformance/test_wire_gml.py`'s existing `PRISM` constant never caught this because it calls `parse_shape()` directly, bypassing schema validation. Decision 123 (Session 18) closes this by extending `DirectPositionListType` to also declare `srsDimension`, admitting the union of GML 3.1.1's geometry-level spelling and the posList-level spelling the parser already read; `REV-PRISM-ENH-001` itself is left on its existing schema-valid `gml:pos`-per-vertex form, since it is decision 122's coverage and should keep testing the form it already tests.
...
**Change:** Added Appendix B decision 124, closing Appendix C.2 item 10. No code changes. The reverse strict interface's existing §10.4 NGUID tiebreak stands unmodified for the case item 10 raised — several reverse candidates carrying different house numbers tied at identical containment, tier, and zero distance because their SSAP records share one coordinate. C.2 item 10's row is marked closed and C.2 is empty again.

**Rationale:** Session 18. Checked item 10's premise against the provisioned Bismarck data before designing a mechanism for it, per this document's own discipline (measurement before tuning, §9 principles). The legitimate case item 10 worried the finding might be confused with — several units of one address sharing a coordinate — does not occur in correctly-provisioned data: STA-006.3's SSAP model places each unit's point independently (`placementMethod: "Parcel"`, confirmed against the 1780 East Calgary Avenue Units 1–6 example already on record), so §10.5's ordinary distance ordering already resolves it without ambiguity, unaided by any merge or spread-test mechanism. What item 10 actually found — distinct civic addresses (different house numbers) sharing one identical coordinate — is a data placement defect, the same posture this document has taken toward prior data-quality findings (the six-county metro artifact, Session 9), and is to be corrected upstream in provisioning rather than compensated for in the scorer. A diagnostic at the tiebreak site was considered and declined as better caught in the GIS provisioning pipeline than surfaced as runtime noise in a service with no way to act on it.

### Version 15 — August 2026

**Change:** Added Appendix B decision 123 and a §16 gap register row. `schemas/gml-geoshape/geometryBasic0d1d.xsd`'s `DirectPositionListType` gains an optional `srsDimension` attribute, admitting `<gml:posList srsDimension="3">` — the GML 3.2/ISO-19136 and common-practice spelling `src/api/wire/gml_xml.py::_ring_vertices` already read, but which stock OGC GML 3.1.1's schema (declaring `srsDimension` only on the geometry element) had no attribute declaration to validate. Both spellings now validate; `AbstractGeometryType`/`SRSReferenceGroup` are untouched. `gml_xml.py`'s runtime `dimension not in (2, 3)` guard remains the actual check against a schema-valid but semantically-unsupported dimension, confirmed by test to still fire after the schema widened. Appendix C.3's Session 17 note on this gap is closed. The §16 row records the gap for external NENA audiences: neither i3 nor RFC 5491 states which GML vintage's `posList` convention a conformant GCS must accept, and 3.1.1 and 3.2 disagree.

**Rationale:** Session 18 (following on from Session 17's decision 122 fixture work), surfaced while adding `REV-PRISM-ENH-001`: a 3D `posList` — the natural rendering of a 3D polygon ring, and the exact form the project's own `PRISM` schema test constant already used — 454'd at admission despite `_ring_vertices` parsing it without complaint. `tests/conformance/test_wire_gml.py`'s `PRISM` test never caught this because it calls `parse_shape()` directly, bypassing schema validation — illustrating why this project's own regression discipline insists on confirming outcomes against the real, fully-validating path. The fix is schema-side because the parser was never the defect: it already implemented the correct, permissive reading. `gcs-pidflo.xsd` and `geometryBasic2d.xsd` were traced and confirmed to hold no independent `DirectPositionListType` declaration, so one edit closes the gap everywhere it's reachable. Recorded in §16, not only Appendix B, because before this decision the service was accidentally stricter than baseline GML 3.1.1 in one specific, narrow way — the attribute present but unrecognized, not a genuine malformation — which is precisely the kind of divergence a NENA integrator would otherwise discover only by trial and error.

### Version 14 — August 2026

**Change:** Added Appendix B decision 122 (§9, §10.3, §12.2). The reverse candidate list is narrowed to the contained candidates where the input shape has extent and at least one candidate falls inside it — `answers()` in `src/reverse/response_assembly.py`, using the existing `SearchOrigin.has_extent` and `ReverseAnswer.contained` values, with no new field and no new configuration variable. §9's enumeration of where the input's extent is carried forward gains §12.2 alongside §10.3 and §10.6; §12.2 gains an amendment note stating the narrowing, the nothing-contained fallback, and the Point-input exemption. `REV-CONTAINED-POLYGON-ENH-001` moves from 154 candidates to 54 and `REV-CIRCLE-ENH-001` from 154 to 49, both reseeded; every strict reverse fixture and both Point-input enhanced fixtures are confirmed unchanged and pinned rather than reseeded. Appendix C.2 reopens with item 10 (the reverse-side co-located contained tie, raised in the same session and deliberately not answered here). Appendix C.3 gains two implementation follow-ups: enhanced fixtures for the five untested input shapes, and the two rank-1 election sites on the reverse side.

**Rationale:** Session 17, raised by the user from `REV-POLYGON-001`'s and `REV-CIRCLE-ENH-001`'s output — a small shape over a known address cluster returned roughly 150 candidates out to the full search radius, most of them demonstrably outside the shape the caller had asserted the target lies within. §9 already held that on the reverse side the input geometry is the query itself and its extent is the substance of the question rather than metadata about it; the code honoured that in the containment flag and the extent-damping term while leaving the candidate SET bounded by the flat radius regardless of shape, so a five-metre circle and a bare Point retrieved identically. Two implementations were considered and rejected before this one: bounding the search by the shape (contradicts §10.1's single pass) and filtering before scoring (the shape of filter decision 61 removed on the forward side). Narrowing at response assembly is decision 118's placement and reasoning applied to the reverse direction. Recorded alongside the decision, because it is easy to misread the change as larger than it is: the narrowing provably cannot move rank 1, since §10.3 already sorts contained candidates ahead of uncontained ones, so the i3 interface's answer is untouched on every fixture and a conformance test now pins that property against a future re-sort.

### Version 13 — August 2026

**Change:** Added Appendix B decision 121, closing Appendix C.2 item 9. `resolve_segment_ambiguity()` / `_segment_spread_m()` (`src/geocode/candidates.py`) extend §6.3's spread-and-refuse test to rung-3 segment geometries, reusing `AmbiguousResult` and `GCS_AMBIGUITY_TOLERANCE_M` rather than adding either a new exception or a second tolerance knob. `response_assembly.strict_answer()` gains `house_number`, threaded from `api/conversion.py::strict_forward_answer()`, and narrows the rung-3 tier on `_rank_segments`'s own key (matchScore, then `range_gap` where a house number is available) before running the spread test — not on matchScore alone — so decision 120's already-settled picks are not reopened. `_range_gap` is renamed `range_gap` (module-public). `FWD-DROPPED-HNO-STRICT-001` moves from a silent single LineString (one of 19 segments spread ~7 km, chosen by NGUID) to 468, and is reseeded. `FWD-GAP-HNO-001` (decision 120's fixture) is confirmed unchanged — a MUST-PASS regression, not a reseed. A new synthetic fixture covers the `range_gap`-tie boundary case. C.2 item 9 is closed.

**Rationale:** Session 17 (same session as decision 120), closing the remainder decision 120 explicitly left open — no house number in the query at all, so `_rank_segments` reduces to `_rank` and every same-named segment ties with no geographic meaning to the NGUID sort deciding among them. A first implementation narrowed the ambiguity check's tier by matchScore alone, matching the task as originally scoped; regression caught that this also caught `FWD-GAP-HNO-001` — 46 segments in the same wide matchScore tie as the no-house-number case, because rung 3's matchScore carries no house-number term regardless of whether one was supplied — silently reopening decision 120's settled pick. The corrected mechanism narrows by the same key `_rank_segments` itself ranks by, catching only the genuinely coarser tie and, as a side effect, the `range_gap`-boundary case neither of the two originally-considered options (apply uniformly vs. scope to no-HNO-only) would have caught.

### Version 12 — August 2026

**Change:** Added Appendix B decision 120 (partial resolution of Appendix C.2 item 9). Rung-3 segment ties are now broken, where the query supplies a usable `Add_Number`, by proximity of each segment's own asserted address range to that number, rather than by NGUID alone — `src/geocode/candidates.py::_rank_segments()`, called from `rcl_candidates()` in place of `_rank()` for the rung-3 list only. C.2 item 9 is narrowed, not closed: it now distinguishes the house-number-present shape (addressed) from the no-house-number shape (`FWD-DROPPED-HNO-STRICT-001`, still open) and additionally notes that a correctly-ordered tie is still a tie §6.3 does not test for. One golden fixture (`FWD-GAP-HNO-001`) moves from a segment ~1,100 house numbers from the query to the nearer of the two segments bracketing the gap, and is reseeded; `FWD-DROPPED-HNO-STRICT-001`/`-ENH-001` are confirmed unchanged.

**Rationale:** Session 17, raised by the user from `FWD-GAP-HNO-001`'s regression output — a query for 1911 Calgary Avenue, a house number falling in a genuine gap between two RoadCenterLine ranges, returned a segment in the 3000 block with no relationship to either bracketing range. Investigation found this was item 9's already-documented tie hazard, reached through a door item 9's own text did not anticipate: a house number that is *present* but *unplaced* ties every same-named segment city-wide (46, in this fixture) at identical rung-3 matchScore, since rung 3 carries no house-number term by construction. Unlike the no-house-number case item 9 was written from, this shape gives the tiebreak something real to compare against — the query's own number — so a narrow fix was judged safe to apply without revisiting the decisions that actually govern this code path (69, 70, 118, 119): none of them bear on a same-rung tiebreak, and the change is confined to `_rank_segments`, leaving `_rank`, rung 1, and rung 2 untouched. The harder, no-house-number shape is deliberately left for its own decision, consistent with decision 119's own choice not to bundle unrelated changes into one reseed.

### Version 11 — August 2026

**Change:** Added Appendix B decision 119 (§3.3, §5, §6.1, §6.2, §6.4). Rung 1 is now inapplicable to a query supplying no usable `Add_Number`, so a street-level query is answered at rung 3 — which §5, decision 63 and §15.1 step 4 have asserted since Session 1 while §6.1's ladder could not produce it. §3.3's ladder table gains the condition on rung 1's row; §6.2 gains the distinction between rung applicability and candidate narrowing (this is neither decision 61's abolished filter nor decision 82's declined administrative gate); §6.4 gains one new path to 468; §15.1 gains step 6a and a note that step 7's short-circuit is unreachable behind it. Two golden fixtures (`FWD-DROPPED-HNO-ENH-001`, `FWD-DROPPED-HNO-STRICT-001`) move from an address-point answer to a street-segment one and are reseeded.

**Rationale:** Session 16, raised by the user from `FWD-DROPPED-HNO-ENH-001`'s regression output and confirmed as a code-versus-specification disagreement rather than a design question — the document asserted the outcome in three places and its own mechanism produced the opposite whenever the SSAP layer held anything on the named street. Recorded at length in the decision itself because the failure needed three independently reasonable rules to coincide (decision 69's gate being conditional, decision 71's gate leaving a clean-looking pair, decision 70's short-circuit being arithmetically correct but blind to correctness as distinct from precision), and a future session re-reading any one of them in isolation will not see it. Two adjacent findings surfaced in the same investigation are recorded there and deliberately left out of it: the Community cascade's unpenalised cross-tier comparison (examined and closed with no change, on decision 80's reasoning) and rung 3's absence of any §6.3 ambiguity test (left open, needing its own decision).

### Version 10 — August 2026

**Change:** Added Appendix B decision 118 (§6.3, §6.4). The i3 interface's §6.3 ambiguity/merge test now runs only within the top-scoring tier of the admitted candidate set (`_TOP_SCORE_TIE_EPSILON`, `src/geocode/response_assembly.py::strict_answer()`) rather than across every candidate that cleared `GCS_MIN_MATCH_SCORE`, closing a real gap the regression suite's own `FWD-AMBIG-MERGE-001` fixture demonstrated: two genuinely co-located candidates 3.9 m apart used to fail 468 because two unrelated, 50+ km-distant candidates that only matched on house number and street name (decision 82's declined administrative gate leaves them admissible) also entered the same spread computation. §6.2 and §6.4 gain settled callouts; `GeocodeEnhanced` is explicitly unaffected. Two golden fixtures (`FWD-AMBIG-MERGE-001`, `FWD-UNIT-DISAMBIG-001`) flip from 468 to 200 and are reseeded; their header comments and `tests/regression/README.md`'s "real data does not do what you expect it to" section are rewritten to describe the corrected outcome rather than the defect that used to produce it.

**Rationale:** Session 15, continuing the same documentation-alignment pass as decisions 116–117 but landing on a genuine behavioral gap rather than a stale claim. The fix deliberately does not add a `GCS_*` environment variable — floated as an option and explicitly declined by the user on the ground that the service already carries enough tuning surface — in favor of a fixed, non-configurable float-tolerance epsilon whose purpose is numerical hygiene (absorbing the same float non-associativity `_weighted_average()`'s docstring already documents from a prior real finding), not an authored editorial judgment about "how close counts as close." Recorded here specifically so a future session does not reach for a new environment variable to solve the same problem without first re-reading why this one wasn't one.
