...
| Normative YAML content types are incoherent                                       | The YAML declares the request body application/json with schema type string, and the 200 response application/xml referencing a JSON-shaped object wrapper (GeodeticData / CivicAddress). An XML content type cannot carry the declared JSON object schema; the definition is unimplementable as written. This implementation emits application/json for the wrapper, carrying the PIDF-LO XML document as the string value (§3.9.1).                                                                                                                                                                                                                                                                               | A future revision of the GitHub definition should declare consistent content types — application/json for the wrapper objects — and state the encoding of the embedded PIDF-LO string.                                                                                          |
