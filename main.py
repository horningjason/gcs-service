"""
GCS Service launcher — reads GCS_TLS_MODE and builds the server-side TLS
context through i3_fe_core.security.tls (spec §3.9.3, §A.8; i3 §2.8.1;
decision 107).
...
ssl_ciphers (confirmed against installed uvicorn==0.46.0's own signature;
i3-fe-core's ADOPTION.md worked examples call `uvicorn.run(..., ssl=ssl_ctx)`,
which is not a real uvicorn.Config parameter). uvicorn's own
create_ssl_context() (uvicorn/config.py) never sets a TLS minimum_version at
all, so passing discrete kwargs cannot express i3 §2.8.1's TLS-1.2 floor or
i3-fe-core's PFS-only cipher suite (security/tls.py's _PFS_CIPHERS_TLS12) —
only a context built by make_server_ssl_context() does both.
...
context to an unmodified uvicorn.Server. This is the same
Config-then-Server sequence `uvicorn.run()` itself reduces to for the
single-process, no-reload, no-multiprocess case this launcher always uses
(uvicorn/main.py's `run()`).
"""
import sys
...
def main() -> None:
    ...
    # no gunicorn arbiter, no reverse proxy in front) — the ENVIRONMENT
    # CAVEAT in i3_fe_core/security/tls.py's module docstring, about
    # CERT_REQUIRED being unreliable behind gunicorn+UvicornWorker, does not
    # apply to this code path. See gunicorn.conf.py for the gunicorn path,
    # which is not this same case and is handled separately.
