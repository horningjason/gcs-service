"""Throwaway PKI for tests/security's live-handshake tests (spec §3.9.3,
decision 107). No real or PCA-issued material — everything here is generated
fresh per test session and thrown away with the tmp directory.
...
def tls_pki(tmp_path_factory: pytest.TempPathFactory) -> TestPKI:
    certs_dir = tmp_path_factory.mktemp("gcs_test_certs")

    ca_key, ca_cert = _self_signed_ca("GCS Test CA — throwaway, not for reuse")
