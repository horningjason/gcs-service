...
i3_fe_core.notify.SipNotifier (i3 §2.4.1 ElementState, §2.4.2 ServiceState).

Ported from lvf-service/tests/test_sip_notifier.py — src/notify/sip_notifier.py
itself is a near-verbatim port of lvf-service's maintained, tested
implementation (not mcs-service's untested copy), so these tests port
one-for-one too rather than being written fresh: this is the coverage
mcs-service skipped when it copied the adapter without copying its tests,
and that gap is exactly what left it unverified.
...
def _make_notifiers():
    identity = ElementIdentity(element_id="gcs.example.com", agency_id="example.com", service_name="GCS")
    ...
    service_notifier = ServiceStateNotifier(
        service="gcs.example.com",
        name="GCS",
        domain="gcs.example.com",
        service_id="gcs.example.com",
        store=store,
...
def _make_adapter_with_logging_client() -> SipWireAdapter:
    """Same as _make_adapter(), but with a real LoggingClient wired through —
    no logging_service_uri, so emit_nowait() never attempts an HTTP POST and
    only exercises the stdlib-logging half every environment can observe."""
    element_notifier, service_notifier = _make_notifiers()
    identity = ElementIdentity(element_id="gcs.example.com", agency_id="example.com", service_name="GCS")
    logging_client = LoggingClient(identity=identity)
    return SipWireAdapter(
        element_notifier, service_notifier, host="127.0.0.1", port=15060,
...
def _make_subscribe(
    ...
    to_uri: str = "sip:gcs.example.com",
...
class TestSubscribeAccessControl:
    def test_unauthorized_subscriber_returns_403(self):
        ...
        async def run():
            with patch.dict(
                os.environ,
                {"GCS_SIP_ALLOWED_SUBSCRIBERS": "sip:allowed@example.com:5060"},
            ):
    ...
    def test_authorized_subscriber_returns_200(self):
        ...
        async def run():
            with patch.dict(
                os.environ,
                {"GCS_SIP_ALLOWED_SUBSCRIBERS": "sip:esrp@192.168.1.1:5060"},
            ):
    ...
    def test_no_allowed_list_accepts_all(self):
        adapter = _make_adapter()
        responses: List[bytes] = []
        env = {k: v for k, v in os.environ.items() if k != "GCS_SIP_ALLOWED_SUBSCRIBERS"}
...
class TestSubscribeLogEvent:
    """logging_client is now wired into SipNotifier construction
    (SipWireAdapter.__init__), matching every other GCS core component
    (element_notifier, service_notifier, discrepancy) — closing the gap
    CLAUDE.md previously flagged. Core owns SubscribeLogEvent (§4.12.3)
    emission entirely; this only confirms the wiring reaches it."""

    def test_subscribe_emits_a_subscribe_log_event(self, caplog):
        adapter = _make_adapter_with_logging_client()
        responses: List[bytes] = []
        ...
        assert responses
        assert Message.parse(responses[0]).code == 200
        ...
        subscribe_events = [e for e in events if e["logEventType"] == "SubscribeLogEvent"]
        assert len(subscribe_events) == 1, f"Expected one SubscribeLogEvent, got {events}"
        assert subscribe_events[0]["package"] == EVENT_ELEMENT
        assert subscribe_events[0]["response"] == 200

    def test_no_logging_client_means_no_subscribe_log_event(self, caplog):
        """_make_adapter() (no logging_client) is the default in every other
        test in this file — confirming it stays silent guards against a
        future change accidentally making logging_client non-optional."""
        adapter = _make_adapter()
        responses: List[bytes] = []
        ...
        assert responses
        assert not any(r.message.startswith("LogEvent: ") for r in caplog.records)
