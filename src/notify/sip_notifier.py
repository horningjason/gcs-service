...
Ported from lvf-service/src/notify/sip_notifier.py (the maintained, tested
implementation — see its 18-test tests/test_sip_notifier.py — rather than
mcs-service's untouched, untested copy). The one substantive adaptation:
lvf-service resolves RFC 4661 filter bodies through src.lost.wire.lost_xml's
shared XML parser; the GCS has no LoST package, so this module defines its
own local _XML_PARSER/_NS_FILTER instead, the same fix mcs-service made for
the same reason.

This closes the i3 §4.5 MUST src/notify/__init__.py's stub docstring
described: ElementState and ServiceState were reachable over HTTP at
/ElementState and /ServiceState but not published over SIP. See
src/app/lifecycle.py's maybe_start_sip() for the leader-gated startup path
and CLAUDE.md for what's still deliberately deferred here (validate_target_uri
is left unset, at parity with both lvf-service and mcs-service — not an
oversight, flagged separately for a follow-up decision). logging_client IS
wired here (runtime_state.logging_client, matching every other GCS core
component) so that every processed SUBSCRIBE emits a SubscribeLogEvent
(§4.12.3) — core's SipNotifier owns the emission logic; this module only
supplies the client.

Configure with:
  GCS_SIP_HOST  — bind address (default 0.0.0.0)
  GCS_SIP_PORT  — bind port (default 5060; set to 0 to disable)
  GCS_SIP_ALLOWED_SUBSCRIBERS — comma-separated permitted Contact URIs (unset = all accepted)
"""
...
class SipWireAdapter:
    ...
    def __init__(
        ...
        self._host = host
        self._port = port
        self._server_uri = os.environ.get("GCS_SERVER_URI", "gcs.example.com")
    ...
    def _authorize(self, request: SipSubscribeRequest) -> bool:
        ...
        allowed_raw = os.environ.get("GCS_SIP_ALLOWED_SUBSCRIBERS", "").strip()
        if not allowed_raw:
            return True
...
# The GCS has no src.lost.wire.lost_xml (no LoST package) to borrow a shared
# parser from, unlike lvf-service — the same reason mcs-service defines its
# own copy of this rather than importing LVF's.
