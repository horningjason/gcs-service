...
# and git (requirements.txt pulls i3-fe-core from a GitHub tag).
...
# Install Python dependencies first (layer caching — only rebuilds if
# requirements change)
...
# ensure the host directory is owned/readable by UID 1000 (gcsuser), e.g.:
#   sudo chown -R 1000:1000 ./data
RUN adduser --disabled-password --gecos "" gcsuser \
    && chown -R gcsuser:gcsuser /app
USER gcsuser
...
# class. Worker count is controlled by GCS_WORKERS (default 1 == single-process
# behavior, identical to `python main.py`).
